...
object AuthenticatedAsync {
  var auth_user = "admin"
  var auth_pw = "temp"

  def setUserPw(user: String, password: String) {
    this.auth_user = user
    this.auth_pw = password
  }

  def Authenticated(action: => String => Request[AnyContent] => Future[SimpleResult]) = {
    Action.async { request => {  // return user or failed crap }
...
object AuthenticatedOld extends ActionBuilder[AuthenticatedRequest] {
  var auth_user = "admin"
  var auth_pw = "temp"
